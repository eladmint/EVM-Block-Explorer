...
use evm_rpc_canister_types::{
    Block, GetBlockByNumberResult, HttpHeader, MultiGetBlockByNumberResult, RpcServices,
    EVM_RPC,
};
...
async fn get_evm_block(height: u128) -> Block {
    // Using Base Mainnet RPC providers since Celo has compatibility issues
...
async fn get_latest_block() -> Block {
    ...
    // Call `eth_get_block_by_number` RPC method with "latest" tag
    let cycles = 10_000_000_000;
    let (result,) = EVM_RPC
        .eth_get_block_by_number(
            rpc_providers,
            None,
            evm_rpc_canister_types::BlockTag::Latest,
            cycles,
        )
        .await
        .expect("Failed to call RPC canister");
